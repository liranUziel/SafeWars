...
export const getTournamentSafes = createAsyncThunk(
	'tournament/getTournamentSafes',
	async ({ user, tournamentId }, thunkAPI) => {
		try {
			return await tournamentService.getTournamentSafe(user, tournamentId);
		} catch (error) {
			const message =
...
export const createTournamentSafes = createAsyncThunk(
	'tournament/createTournamentSafes',
	async ({ user, classId, showScore, deadLine }, thunkAPI) => {
		try {
